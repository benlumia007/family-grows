...
function family_grows_entry_posted_on_setup() {
    ...
    esc_attr(sprintf(__('View all posts by %s', 'family-grows'), get_the_author())), 
    get_the_author()
    ));

    if ( !is_page() && !post_password_required() && (comments_open() || get_comments_number())) {
        echo '<span class="entry-comments"><b>';
            comments_popup_link( sprintf( __( 'Add a Comment', 'family-grows')));
        echo '</b></span>';
    }
}
...
function family_grows_entry_time_stamp_setup() {
	...
	return sprintf(
		__('<span class="screen-reader-text">Posted on</span> %s', 'family-grows'),
...
function family_grows_entry_taxonomies_setup() {
    ...
    if ($cat_list) {
        ...
        __('<i class="fa fa-folder-open-o"></i> Posted In', 'family-grows'),  
        $cat_list
        );
    }
    ...
    if ($tag_list) {
        ...
        __('<i class="fa fa-tags"></i> Tagged', 'family-grows'),  
        $tag_list 
        );
    }
}